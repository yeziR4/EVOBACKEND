    ...
    try:
        ...
        def abandoned_artists_or_genres():
            ...
            abandoned_artists = {
                ...
                if sum(artist in artists for artists in artists_by_year.values()) == 1
            }
            ...
            abandoned_genres = {
                ...
                if sum(genre in genres for genres in genres_by_year.values()) == 1
            }

            return {
                "artists": list(abandoned_artists), 
                "genres": list(abandoned_genres)
            }

        def most_consistent_songs():
            """Find songs appearing every year."""
            songs_by_year = {
                year: set(track["name"] for track in tracks) 
                for year, tracks in tracks_by_year.items()
            }
